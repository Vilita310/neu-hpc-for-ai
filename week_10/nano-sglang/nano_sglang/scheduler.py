...
class Scheduler:
    ...
    def _prefill_waiting(self, sampling_params: SamplingParams):
        ...
        # If caller provides run-level max_tokens, apply it for requests that
        # were enqueued without an explicit per-request override.
        if seq.max_tokens == SamplingParams().max_tokens:
        ...
        if first_token == self.tokenizer.eos_token_id or seq.num_generated >= seq.max_tokens:
            seq.status = SequenceStatus.FINISHED
            self.finished.append(seq)
        else:
    ...
    def run_to_completion(self, sampling_params: SamplingParams = None) -> list[str]:
        ...
        outputs = sorted(self.finished, key=lambda s: s.seq_id)
        results = [self.tokenizer.decode(seq.output_token_ids) for seq in outputs]
